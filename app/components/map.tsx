...
export const Map = () => {
  ...
  useEffect(() => {
    ...
    pins.forEach((pin) => {
      map.addPin({
        ...pin,
        svgOptions: { color: "rgba(139, 92, 246, 0.8)", radius: 0.15 },
      });
    });
    ...
    const src = `data:image/svg+xml;utf8,${encodeURIComponent(svgMap)}`;
    queueMicrotask(() => setSvgSrc(src));
  }, []);

  if (!svgSrc)
    return (
      <div
        className="h-[150px] w-full animate-pulse rounded-xl"
